                ...
                <form role="form" class="form-horizontal" method="POST" action="{{$form_url}}">
                            ...
                            <?php } else { ?>
                                <select name="provinsi" class="form-control provinsi">
                                    <?php foreach ($provinsi as $prov) { ?>
                                        ...
                                        <option value="<?php echo $prov->id ?>"
                                        <?php
                                        if ($record->kecamatan->kabkota->provinsi->nama == $prov->nama) {
                                ...
                                <select name="kabkota" class="form-control kabkota">
                                    <?php foreach ($kabkota as $kk) { ?>
                                        <option value="<?php echo $kk->id ?>"
                                                <?php if ($record->kecamatan->kabkota->nama == $kk->nama) echo 'selected="selected"'; ?>>
                                                    <?php echo $kk->nama; ?>
                                        </option>
                                    <?php } ?>
                                </select>
                                ...
                                <select name="kecamatan" class="form-control kecamatan">
                                    <?php foreach ($kecamatan as $kec) { ?>
                                        <option value="<?php echo $kec->id ?>"
                                                <?php if ($record->kecamatan->nama == $kec->nama) echo 'selected="selected"'; ?>>
                                                    <?php echo $kec->nama; ?>
                                        </option>
                                    <?php } ?>
                                </select>
                    ...
                    <script type="text/javascript">
                        $('.provinsi').on('change', function() {
                            $.ajax({
                                type: 'GET',
                                url: "<?php echo URL::to('lpantb/desa/getkabkota') ?>/" + this.value,
                                dataType: 'json',
                        ...
                        $('.kabkota').on('change', function() {
                            $.ajax({
                                type: 'GET',
                                url: "<?php echo URL::to('lpantb/desa/getkecamatan') ?>/" + this.value,
                                dataType: 'json',
                                success: function(data) {
                                    var html = [];
                                    for (var i = 0, len = data.length; i < len; i++) {
                        ...
                        <div class="col-sm-5">
                            <?php if ($form_status == "add") { ?>
                                <input type="text" class="form-control" 
                                       name="desa" placeholder="Nama Desa"
                                       required="required">
                                   <?php } else { ?>
                                <input type="text" class="form-control" 
                                       name="desa" value="{{$record->nama}}"
                                       required="required">
                                   <?php } ?>
                        </div>
